...
def band_pass_filter(data, low_freq, high_freq, sfreq, order=5):
    ...
    if low is None and high is None:
        ...
        if not 0 < low < high < 1:
            raise ValueError(
                f"Frequencies must satisfy 0 < low ({low_freq} Hz) < high ({high_freq} Hz) < Nyquist ({nyquist} Hz)"
            )
...
def filter_data_signal(eeg_data, eeg_channels):
    filtered_data = {
        band: {str(channel): None for channel in eeg_channels}
        for band in frequency_bands
    }
    for band, (low_freq, high_freq) in frequency_bands.items():
        for channel in eeg_channels:
            filtered_data[band][channel] = band_pass_filter(
                eeg_data[channel], low_freq, high_freq, sfreq
            )
    return filtered_data
